...
// 'Timm' suchen um Änderungen zu sehen
...
CLASS TControl FROM TWindow
   ...
   METHOD Set3DLook( lOnOff ) INLINE Ctl3DLook( ::hWnd, lOnOff )
...
METHOD KeyDown( nKey, nFlags ) CLASS TControl
   ...
   /*Timm: Tastencodes werden vom Programm abgefangen
   if nKey == VK_ESCAPE
           ...
           if ! Empty( oFolder := ::oWnd:oWnd ) .and. ;
              ...
              if ( nItem := oFolder:GetHotPos( nLoWord ) ) > 0
                 ...
                 if ( hCtrl := ::oWnd:GetHotPos( nLoWord, ::hWnd ) ) != 0
                    ...
                    if nButton != 0
                       if ! ::oWnd:aControls[ nButton ]:lCancel
                          SetFocus( hCtrl )
                       endif
...
METHOD __HelpTopic() CLASS TControl
   ...
   if Empty( ::nHelpId )
      return nil  // WM_HELP is passed to its parent
   else
      HelpPopup( ::nHelpId )
...
METHOD ShowDots( lNotShow ) CLASS TControl
   ...
   IF GetKeyState( VK_SHIFT ) = .T. .AND. lNotShow = .F.
      lShow := .F.
   ENDIF
   //EndTimm

   if ::aDots != nil .AND. lShow = .T. //Timm
...
METHOD SysChar( nKey, nFlags ) CLASS TControl

   local n, cPrompt, nAt

   if Upper( ::oWnd:ClassName() ) == "TMDICHILD"
      for n = 1 to Len( ::oWnd:aControls )
         cPrompt = GetWindowText( ::oWnd:aControls[ n ]:hWnd )
         nAt = At( "&", cPrompt )
         if nAt != 0
