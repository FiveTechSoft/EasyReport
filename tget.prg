...
// "Timm": Insert-Modus bei Get-Feldern
//         Focused Get erhält hervorhebende Hintergrundfarbe
...
METHOD New( nRow, nCol, bSetGet, oWnd, nWidth, nHeight, cPict, bValid,;
   ...
   IF oGenVar:nFocusGetBackClr <> 0
      ::bGotFocus  := {|| ::SetColor( ::nClrText, oGenVar:nFocusGetBackClr ) }  // Timm
      ::bLostFocus := {|| ::SetColor( ::nClrText, nClrBack ) }                  // Timm
   ENDIF
...
METHOD KeyDown( nKey, nFlags ) CLASS TGet
   ...
   do case
      ...
      case nKey == VK_INSERT .and. ! GetKeyState( VK_SHIFT ) ;
           ...
           DestroyCaret()
           if Set( _SET_INSERT )
              CreateCaret( ::hWnd, 0, 1, ::nGetChrHeight() - 1 )
              //Timm CreateCaret( ::hWnd, 0, 6, ::nGetChrHeight() - 1 )
           else
              CreateCaret( ::hWnd, 0, 1, ::nGetChrHeight() - 1 )
              //Timm CreateCaret( ::hWnd, 0, 2, ::nGetChrHeight() )
           endif
           ShowCaret( ::hWnd )
           return 0
